...
export const POST: RequestHandler = async ({ request, locals, platform }) => {
  ...
  try {
    ...
    // === NEW === R2 + public base from CF env
    const bucket = (platform?.env)?.R2_BUCKET as R2Bucket | undefined;
    const publicBase = (platform?.env)?.R2_PUBLIC_BASE as string | undefined;
    if (!bucket) return errorResponse('R2 bucket not bound', 500);
